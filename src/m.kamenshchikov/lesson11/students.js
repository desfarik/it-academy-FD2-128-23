...
function sortsByFullName(students) {
  return students.slice().sort(function (student1, student2) {
...
function showStudentsToAutomat(lessons, allStudents) {
  const studentsToAutomat = allStudents.filter((student) =>
    lessons.every((lesson) => lesson.isStudentVisit(student))
  );
...
function showMissedLessonsStudents(lessons, students) {
  let data = students
    .map((student) => {
      const visitedLessons = lessons.filter((lesson) =>
        lesson.isStudentVisit(student)
      );
      return {
...
function showStudentsVisit(lessons, students) {
  let tableData = sortsByFullName(students).map((student) => {
    let rowData = {
      name: student.getFullName(),
    };
